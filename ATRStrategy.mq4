...
extern string I_6  = "----------------------- 'ATR Strategy' indicator settings --------------------------"; //.
...
bool closeBuy()
  {
   bool retVal = false;
   if(pivot != Atr(0,1) || (Close[1] < Atr(1,1)))
     {
      retVal = true;
     }
   return retVal;
...
bool closeSell()
  {
   bool retVal = false;
   if(pivot != Atr(0,1) || (Close[1] > Atr(0,1)))
     {
      retVal = true;
     }
   return retVal;
...
//|                                                                  |
//+------------------------------------------------------------------+
double Atr(int Buffer,int Shift)
  {
   double retVal = iCustom(NULL,0,"ATRcta",kValue,Buffer,Shift);
   return retVal;
  }
//+------------------------------------------------------------------+
